...
%                      Chernoff bound
%    'roc'             Area under the empirical receiver operating
...
%
%   See also CLASSIFY, CLASSPERF, CROSSVALIND, RANDFEATURES, SQTLFEATURES,
%   SVMCLASSIFY.

%   Copyright 2003-2005 The MathWorks, Inc.
%   $Revision: 1.1.8.1 $  $Date: 2005/06/09 21:58:16 $
        ...
        elseif length(k)>1
            ...
            switch(k)
                case 1 % criterion
                    ...
                    method = strmatch(lower(pval),methods); %#ok
                    if isempty(method) 
...
if alpha == 0 && ~betaIsHandle && beta==0
    ...
    sumXsq = sum(X.^2,2);
    [m,idx(1)]=max(z); %#ok %most significant features
    for n = 1:numIndices-1
        ...
        % choose next feature
        [m,idx(n+1)] = max(z .* weights1 .* weights2); %#ok
    end
end

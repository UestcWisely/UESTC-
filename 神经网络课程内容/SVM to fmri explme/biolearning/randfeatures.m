...
%
%   See also CLASSIFY, CLASSPERF, CROSSVALIND, RANKFEATURES, SVMCLASSIFY.
 
%   Copyright 2003-2005 The MathWorks, Inc.
%   $Revision: 1.1.10.3 $  $Date: 2005/06/17 20:19:36 $
...
if nargin < 2
    error('Bioinfo:randfeatures:NotEnoughInputs','Requires at least three arguments.');
end
...
if length(todel) > 0  % remove observations not pre-classified
    X(todel,:) = [];
    group(todel) = [];
    ...
    for j=1:2:nvarargin
        ...
        elseif length(k)>1
            ...
            switch(k)
                case 1 % classifiers
                    classifiers = {'knn','da'};
                    classifier = strmatch(lower(pval),classifiers); %#ok
                ...
                case 8 % cross normalization
                    ...
                    cnorm = strmatch(lower(pval),cnorms); %#ok
                    if isempty(cnorm) 
...
while (numGood < poolSize)
    ...
    switch classifier
        ...
        otherwise % linear or quadratic
            [c,err,pos] = classify(X(thePerm,:)',X(thePerm,:)',group,daOptions{:});%#ok
